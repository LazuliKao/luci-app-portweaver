...
;// CONCATENATED MODULE: ./components/StatusPanel.ts
...
class StatusPanel {
    ...
    constructor(E, _){
        _define_property(this, "E", void 0);
        _define_property(this, "_", void 0);
        this.E = E;
        this._ = _;
    }
}

;// CONCATENATED MODULE: ./components/FrpNodeSelector.ts
function createFrpNodeSelector(form, uci, E, _) {
...
;// CONCATENATED MODULE: ./components/PortMappingEditor.ts
function createPortMappingEditor(form, uci, E, _) {
    return form.DummyValue.extend({
        ...
        renderWidget: function(section_id, option_index, cfgvalue) {
            var frp_sections = uci.sections('portweaver', 'frp_node') || [];
            var current_values = Array.isArray(cfgvalue) ? cfgvalue : typeof cfgvalue === 'string' ? String(cfgvalue).split(/\s+/).filter(Boolean) : [];
            var widget_id = this.cbid(section_id);
            var self = this;
            var container = E('div', {
                'class': 'cbi-value-field'
            });
            var mappings_wrapper = E('div', {
            ...
            var renderMappingRow = function(mapping_str, index) {
                ...
                var frpContainer = E('div', {
                    'class': 'frp-nodes-select',
                    'style': 'margin-top: 10px; padding: 10px; background: #f0f0f0; border-radius: 3px; display: none;'
                });
                if (frp_sections.length > 0) {
                    frpContainer.appendChild(E('label', {
                    ...
                    frp_sections.forEach(function(frp_section) {
                        ...
                        if (!node_name) return;
                        var is_checked = (mapping.frpNodes || []).some(function(n) {
                            return n.split(':')[0] === node_name;
                        });
                        var port_value = '';
                        if (is_checked) {
                            var found = (mapping.frpNodes || []).find(function(n) {
                                return n.split(':')[0] === node_name;
                            });
                            if (found) {
                ...
                listenInput.addEventListener('input', function() {
                    updatePreview();
                    updateHiddenValue();
                });
                listenInput.addEventListener('change', function() {
                    updatePreview();
                    updateHiddenValue();
                });
                ...
                textModeInput.addEventListener('input', function() {
                    ...
                    if (parsed) {
                        ...
                        allCheckboxes.forEach(function(cb) {
                            var node = cb.getAttribute('data-node');
                            var is_checked = (parsed.frpNodes || []).some(function(n) {
                                return n.split(':')[0] === node;
                            });
                            ...
                            if (port_inp) {
                                ...
                                if (is_checked) {
                                    var found = (parsed.frpNodes || []).find(function(n) {
                                        return n.split(':')[0] === node;
                                    });
                                    if (found) {
                                        var parts = found.split(':');
...
// LuCI require headers are injected by rsbuild banner config
// See rsbuild.config.ts output.banner for the actual require statements
...
const viewModule = view.extend({
    ...
    render: function(data) {
        ...
        m = new form.Map('portweaver', _('PortWeaver'), _('Port forwarding and NAT traversal configuration'));
        // Setup auto-refresh
        poll.add(function() {
        ...
        var runtimeToggle = function(section_id) {
            ...
            return rpcClient.setEnabled(idx, !!newEnabled).then(function() {
                ...
                ui.addNotification(null, E('p', _('Failed to toggle runtime state: ') + ((err === null || err === void 0 ? void 0 : err.message) || String(err))), 'error');
            });
        };
        window.portweaverToggle = runtimeToggle;
        // Port forwarding rules section
        s = m.section(form.GridSection, 'project', _('Port Forwarding Projects'), _('Configure port forwarding projects for PortWeaver'));
        s.anonymous = true;
        s.addremove = true;
        s.sortable = true;
        s.cloneable = true;
        ...
        o.default = '1';
        o.depends('enable_app_forward', '1');
        o = s.option(form.Flag, 'enable_stats', _('Enable Statistics'), _('Collect traffic statistics (bytes_in/bytes_out) using zero-cost atomic counters. ' + 'NOTE: Mutually exclusive with firewall forwarding - enabling stats will disable add_firewall_forward.'));
        o.modalonly = true;
        o.default = '0';
        o.depends('enable_app_forward', '1');
        ...
        s = m.section(form.GridSection, 'frp_node', _('FRP Node Management'), _('Configure FRP nodes for port forwarding tunneling'));
        s.anonymous = true;
        s.addremove = true;
        s.sortable = true;
        s.cloneable = true;
...
/* export default */ const main = ((/* unused pure expression or super */ null && (viewModule)));
