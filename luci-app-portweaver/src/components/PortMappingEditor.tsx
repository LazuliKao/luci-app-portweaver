export function createPortMappingEditor(form: any, uci: any) {
  return form.DummyValue.extend({
    ...
    renderWidget: function (section_id: string, _option_index: number, cfgvalue: string[] | string) {
      void _option_index;
      ...
      const self = this;
      const mappings_wrapper = <div id={`portmapping-wrapper-${section_id}`}></div> as HTMLElement;

      const updateHiddenValue = (): void => {
        const rows = mappings_wrapper.querySelectorAll('.portmapping-row');
        const values: string[] = [];
        rows.forEach((r: Element) => {
          const listen = (r.querySelector('.listen-port-input') as HTMLInputElement).value.trim();
          const target = (r.querySelector('.target-port-input') as HTMLInputElement).value.trim();
          const protocol = (r.querySelector('.protocol-select') as HTMLSelectElement).value;
          ...
          checkboxes.forEach((cb: any) => {
            if (cb.checked) {
              ...
              const port_inp = r.querySelector('input.frp-node-port-pm[data-node="' + node + '"]') as HTMLInputElement | null;
              const port = port_inp ? port_inp.value.trim() : '';
              frpNodes.push(port ? node + ':' + port : node);
            }
          });
          const temp = { listenPort: listen, targetPort: target, frpNodes: frpNodes, protocol: protocol };
          const str = self.buildString(temp);
          if (str && listen && target) values.push(str);
        });
        const hidden = document.getElementById('portmapping-hidden-' + section_id) as HTMLInputElement | null;
        if (hidden) hidden.value = values.join(' ');
      };

      const renderMappingRow = (mapping_str: string, index: number): HTMLElement => {
        const mapping = self.parseMapping(mapping_str) || { listenPort: '', targetPort: '', frpNodes: [], protocol: 'tcp' };
        ...
        const updatePreview = (): void => {
          ...
          const temp_mapping = { listenPort: listen, targetPort: target, frpNodes: frpNodes, protocol: protocol };
        ...
        const frpContainer = (
          <div class="frp-nodes-select" style="margin-top: 10px; padding: 10px; background: #f0f0f0; border-radius: 3px; display: block;"></div>
        ) as HTMLElement;

        if (frp_sections.length > 0) {
          frpContainer.appendChild(
            <label style="display: block; margin-bottom: 8px; font-weight: bold;">{_('FRP Nodes (Optional):')}</label>
          );
          ...
          frp_sections.forEach((frp_section: any) => {
            ...
            const handlePortChange = (): void => {
              ...
              if (port) {
                ...
                if (isNaN(p) || p < 1 || p > 65535) {
                  port_input.style.setProperty('border-color', 'red', 'important');
                } else {
                  port_input.style.borderColor = '';
                }
              } else {
            ...
            frpContainer.appendChild(
              <div style="margin-bottom: 5px;">
                {checkbox}
                <label style="margin-right: 10px; cursor: pointer;">{node_name}</label>
          ...
          frpContainer.appendChild(<em style="color: #999;">{_('No FRP nodes configured')}</em>);
        }

        const errorDiv = (
          <div class="portmapping-error" data-index={index} style="color: red; margin-top: 8px; min-height: 20px; font-size: 12px;"></div>
        ) as HTMLElement;

        const titleRow = (
          <div style="display: flex; gap: 10px; align-items: center;">
            <label style="min-width: 80px; font-weight: bold;">{_('Listen Port:')}</label>
            {listenInput}
            <label style="min-width: 80px; font-weight: bold;">{_('Target Port:')}</label>
            {targetInput}
            <label style="min-width: 60px; font-weight: bold;">{_('Protocol:')}</label>
            {protocolSelect}
          </div>
        ) as HTMLElement;
        ...
        textModeInput.oninput = (ev: Event) => {
          ...
          if (parsed) {
            ...
            allCheckboxes.forEach((cb: any) => {
              ...
              const is_checked = (parsed.frpNodes || []).some((n: string) => n.split(':')[0] === node);
      ...
      const hidden = <input type="hidden" id={`portmapping-hidden-${section_id}`} name={widget_id} value={current_values.join(' ')} />;
